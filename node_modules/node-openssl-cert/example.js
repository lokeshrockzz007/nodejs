...
/*openssl.generateRSAPrivateKey(rsakeyoptions, function(err, key, cmd) {
	...
	openssl.generateCSR(csroptions, key, 'test test', function(err, csr, cmd) {
			if(err) {
					...
					//console.log(cmd.files.config);
					csroptions.days = 240;
					openssl.selfSignCSR(csr, csroptions, key, 'test test', function(err, crt, cmd) {
							if(err) {
									console.log(err);
									console.log(cmd.files.config);
							} else {
...
/*
fs.readFile('./test/encrypted.key', function(err, contents) {
    openssl.importRSAPrivateKey(contents, 'test test', function(err, key, cmd) {
	//console.log(cmd);
		if(err) {
			console.log(err);
		} else {	
